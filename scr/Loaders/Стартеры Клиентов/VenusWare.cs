...
namespace Loaders
{
    public partial class VenusWare : Form
    {
        private bool isRunning = false; // Переменная для отслеживания запуска
        private readonly string logPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "AourusLaucher", "Logs", "wexside.logs");
        ...
        private readonly VersionInfo[] versions = new VersionInfo[]
        {
            new VersionInfo
            {
                Name = "VenusWare 1.16.5",
                Folder = "VenusWare",
                DownloadUrl = "http://213.176.115.153:8080/CheatClients/VenusWare.zip",
